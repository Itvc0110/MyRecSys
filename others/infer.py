...
if __name__ == "__main__":
    ...
    os.makedirs(project_root / "other" / "result", exist_ok=True)
    os.makedirs(project_root / "other" / "infer_data", exist_ok=True)

    infer_user_other_path = os.path.join(project_root, "other/infer_data/infer_user_other.csv")
    result_json_path = os.path.join(project_root, "other/result/result.json")
    rulename_json_path = os.path.join(project_root, "other/result/rulename.json")
    content_other_path = os.path.join(project_root, "other/infer_data/merged_content_others.csv")
    rule_content_path = os.path.join(project_root, "other/result/rule_content.txt")
    ...
    if os.path.exists(infer_user_other_path):
        infer_user_other_df = pd.read_csv(infer_user_other_path)
    else:
        process_infer_data(user_data_path, other_data_path, 10, 10000, infer_user_other_path)
        infer_user_other_df = pd.read_csv(infer_user_other_path)

    infer_user_other_df = infer_user_other_df.fillna(0)
    exclude = {'username', 'content_id', 'profile_id'}
    to_convert = [col for col in infer_user_other_df.columns if col not in exclude]
    infer_user_other_df[to_convert] = infer_user_other_df[to_convert].apply(pd.to_numeric, errors='coerce')
    infer_user_other_df = infer_user_other_df.dropna()
    infer_user_other_df = infer_user_other_df.astype({col: 'float32' for col in infer_user_other_df.columns if col not in ['username', 'content_id', 'profile_id']})

    interaction_df = infer_user_other_df[['username', 'content_id', 'profile_id']]
    infer_df = infer_user_other_df.drop(columns = ['username', 'content_id', 'profile_id'], axis=1)
    ...
    checkpoint_path = "model/other/best_model.pth"              
    ...
    content_other_df = pd.read_csv(content_other_path)
