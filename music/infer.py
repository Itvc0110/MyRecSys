...
if __name__ == "__main__":
    ...
    os.makedirs(project_root / "music" / "result", exist_ok=True)
    os.makedirs(project_root / "music" / "infer_data", exist_ok=True)

    # File paths
    user_data_path = os.path.join(project_root, "month_mytv_info.parquet")
    music_data_path = os.path.join(project_root, "mytv_vmp_content")
    processed_user_path = os.path.join(project_root, "music/infer_data/user_data.parquet")
    processed_item_path = os.path.join(project_root, "music/infer_data/music_item_data.parquet")
    content_music_path = os.path.join(project_root, "music/infer_data/merged_content_musics.parquet")
    tags_path = os.path.join(project_root, "tags")
    rule_info_path = os.path.join(project_root, "rule_info.parquet")
    result_json_path = os.path.join(project_root, "music/result/result.json")
    rulename_json_path = os.path.join(project_root, "music/result/rulename.json")
    rule_content_path = os.path.join(project_root, "music/result/rule_content.txt")

    # Load model
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    checkpoint_path = os.path.join(project_root, "model/music/best_model.pth")
    ...
    user_profile_df, music_pl, content_dict = process_infer_data(processed_user_path, user_data_path, processed_item_path, music_data_path, content_music_path, num_user=-1, num_music=-1)
    ...
    for i in range(num_chunks):
        ...
        cross_df = user_chunk_pl.join(music_pl, how="cross")
        ...
        with open(project_root / f"music/result/result_chunk_{i+1}.json", 'w', encoding='utf-8') as f:
            json.dump(result_with_rule, f, indent=4, ensure_ascii=False)
        with open(project_root / f"music/result/rulename_chunk_{i+1}.json", 'w', encoding='utf-8') as f:
            json.dump(homepage_rule, f, indent=4, ensure_ascii=False)
        with open(project_root / f"music/result/rule_content_chunk_{i+1}.txt", "w", encoding='utf-8') as f:
    ...
    final_rule_content = ""
    for i in range(num_chunks):
        with open(project_root / f"music/result/result_chunk_{i+1}.json", 'r', encoding='utf-8') as f:
            final_result.update(json.load(f))
        with open(project_root / f"music/result/rulename_chunk_{i+1}.json", 'r', encoding='utf-8') as f:
            final_rulename.extend(json.load(f))
        with open(project_root / f"music/result/rule_content_chunk_{i+1}.txt", 'r', encoding='utf-8') as f:
