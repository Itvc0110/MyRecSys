...
model_save_dir = os.path.join(Path().resolve(), "model/livestream")
...
def train(model, train_loader, val_loader, optimizer, loss_fn, device, epochs=10, patience=3):
    ...
    model_path = "model/livestream/best_model.pth"
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            if best_val_loss < best_previous_loss:
                save_checkpoint(model, epoch, optimizer, best_val_loss, path="model/livestream/best_model.pth")
        else:
...
if __name__ == "__main__":
    project_root = Path().resolve()
    data_path = "livestream/train_data/merged_user_item_duration.parquet"
    ...
    print_class_distribution(pd.livestream(y_train), "Training")
    print_class_distribution(pd.livestream(y_val), "Validation")
